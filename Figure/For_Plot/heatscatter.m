function [OutX,OutY] = heatscatter(X, Y, outpath, outname, numbins, markersize, marker, plot_colorbar, plot_lsf, xlab, ylab, title)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     f = figure();
    ...
    if (plot_lsf)
        [m,idx] = max(values,[],2);
        c = fit(centers_X',(idx-1)*binsize_Y*2+centers_Y(1),'poly3');
        hold on
        plot(c,centers_X',(idx-1)*binsize_Y*2+centers_Y(1),'.k')
...
%     saveas(f, outfile);
    OutX=centers_X';
    OutY=(idx-1)*binsize_Y*2+centers_Y(1);
    fprintf(' Done!\n');
