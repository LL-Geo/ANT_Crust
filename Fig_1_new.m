...
legend off
xlabel('log_{10}(density(kg m^{-3}))','Position',[0.475 -3.4])
...
mean(mis_hp,'omitnan')
std(mis_hp,'omitnan')
median(mis_hp,'omitnan')


mean(mis_den,'omitnan')
std(mis_den,'omitnan')
median(mis_den,'omitnan')

figure()
subplot(2,2,1)
scatter(x,y,[],HP_petro,'filled')
caxis([0,3])
subplot(2,2,2)
scatter(x,y,[],hp_inv_interp,'filled')
caxis([0,3])

subplot(2,2,3)
scatter(x,y,[],mis_hp,'filled')
caxis([-2,2])
colorcet('D01A')
subplot(2,2,4)
plot(HP_petro,hp_inv_interp,'.')
hold on
plot([0,3],[0,3])
axis equal
